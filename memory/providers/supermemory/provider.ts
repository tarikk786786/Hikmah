...
export class SupermemoryProvider implements MemoryProvider {
  public readonly id = 'supermemory';
  public readonly name = 'Supermemory (Large-Scale Document & Web Corpus)';
  public readonly primaryClassifications: MemoryClassification[] = ['DOCUMENT'];
  ...
  constructor(apiKey?: string, endpoint?: string) {
    ...
    this.endpoint = endpoint || process.env.SUPERMEMORY_ENDPOINT;
  }

  public async remember(input: MemoryWriteInput): Promise<UnifiedMemoryRecord> {
    const id = `mem_smem_${uuidv4().substring(0, 8)}`;
    ...
    const record: UnifiedMemoryRecord = {
      ...
      authority: input.authority || 'DOCUMENT',
      provider: this.id,
      providerRecordId: `smem_doc_${id}`,
      importance: input.importance || 6.5,
